...
module.exports = {
    // 查询配置
    'GET /api/config/workshop': (req, res) => {
        res.send({
            data: defaultConfig,
            success: true
        })
    ...
    // 删除配置
    'POST /api/config/workshop/del': (req, res) => {
    ...
    // 编辑或添加配置
    'POST /api/config/workshop/edit': (req, res) => {
        ...
        keys.forEach(rowKey => {
            ...
            if (findIndex > -1) {
                ...
                if (newConfig[rowKey].name && newConfig[rowKey].number) {
