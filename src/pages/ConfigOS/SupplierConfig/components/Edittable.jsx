                    ...
                    onSave: async (a, the_change) => {
                        ...
                        props.data.map(val => {
                            if (the_change.id === val.id) {
                                if (val.supplier_type) {  //组件复用时判断是类型还是级别
                                    val.supplier_type = the_change.supplier_type
                                    props.update_type(['update', val])
                                    flag = false;  //flase,就不进行下面的添加操作
                                }
                                if (val.supplier_level) {
                                    val.supplier_level = the_change.supplier_level
                                    props.update_type(['update', val])
                                    flag = false;
                                }
                            }
                        ...
                        if (flag) {
                            //添加操作
                            if (the_change.supplier_type) {
                                props.update_type(['add', {
                                    id: the_change.id,
                                    supplier_type: the_change.supplier_type
                                }])
                            }
                            else if (the_change.supplier_level) {
                                props.update_type(['add', {
                                    id: the_change.id,
                                    supplier_level: the_change.supplier_level
                                }])
                            }
                        ...
                        <a
                            ...
                            onClick={
                                () => {
                                    ...
                                    if (!config.form.getFieldsValue()[row.id].supplier_type || !config.form.getFieldsValue()[row.id].supplier_level) {
                                        config.form.validateFields(); //解决新增的时候直接点击保存不校验是否位空的错误
                                    }

                                    if (config.form.getFieldsValue()[row.id].supplier_type || config.form.getFieldsValue()[row.id].supplier_level) {
                                        let newdata = {}
                                        if (config.form.getFieldValue()[row.id].supplier_type) {
                                            newdata = {
                                                id: row.id,
                                                supplier_type: config.form.getFieldValue()[row.id].supplier_type
                                            }
                                        }
                                        else if (config.form.getFieldValue()[row.id].supplier_level) {
                                            newdata = {
                                                id: row.id,
                                                supplier_level: config.form.getFieldValue()[row.id].supplier_level
                                            }
                                        }
